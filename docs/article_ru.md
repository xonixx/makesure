...
Хотя, например, https://github.com/adriancooney/Taskfile представляет прекрасный шаблон для подобных скриптов.

Так и родился makesure TODO LINK

Что это?

По сути это поименованные кусочки shell (называемые целями), объединённые в одном файле. Это позволяет легко перечислить цели (с пояснительным текстом) и вызвать любую из них по имени.
Да, вот так просто, весьма похоже на то, что позволяют скрипты package.json в nodejs.

Но это ещё не все. Цели могут декларировать TODO link @depends_on зависимости от других целей и makesure будет учитывать это при выполнении. Это поведение весьма близко к оригинальному make. Цель также может декларировать условие того что она уже достигнута TODO link to @reached_if. В этом случае тело цели (соответствующий shell скрипт) уже не будет исполняться. Этот простой механизм позволяет очень удобно и декларативно выражать идемпотентную TODO LINK логику работы, а попросту говоря ускорять сборку, так как то что уже выполнено не будет выполняться повторно. Эта фича уже была основана на идеях из Ansible.
