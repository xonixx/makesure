...
Изначально я хотел спроектировать эту часть в наиболее общем смысле. Так, я решил, что в инструменте будет понятие prelude - это скрипт, идущий перед всеми декларациями `@goal`. Целью этого скрипта будет инициализация глобальных переменных. Ну какой-нибудь такой гипотетический пример
...
Пару отягчающих моментов. Во-первых, следует принять во внимание, что под капотом каждый из @goal-скриптов выполняется в отдельном процессе shell. Сделано это намеренно, чтобы исключить возможность зависимостей через глобальные переменные между целями, что может сделать логику исполнения более императивной и запутанной.  

Во-вторых, должна быть возможность переопределить значение переменной в момент запуска, например так

```
./makesure -D VERSION=0.0.2 build
```

В третьих, хотелось чтобы prelude-скрипт исполнялся только единожды, не зависимо от того сколько целей будет исполнено в процессе.
